...
1. **GitHub 사용자명 교체**: `theme.config.tsx` 와 `README.md` 안의 `thenext0202` 을 본인 GitHub 사용자명으로 바꿔주세요.
2. **경로**: 프로젝트 경로는 **반드시 ASCII 영문 경로**에 두세요. 한글 경로(`D:\개발\...` 등)에서는 webpack `readlink` 이슈로 production build가 실패합니다. dev 서버는 영문 경로에서 정상 동작합니다.
3. **Node.js 18.17 이상** 권장.
...
각 섹션은 `_meta.ts` 로 사이드바 순서를 관리합니다.
...
### Windows 로컬에서 `npm run build` 실패: `EISDIR ... readlink ...`

원인: Nextra 3.3.x가 페이지 메타를 스캔할 때 webpack이 일부 Windows 환경에서 `readlink`를 잘못 호출하는 알려진 이슈.

- **로컬 개발은 `npm run dev`** 로 진행하세요 (정상 동작 확인됨).
- **프로덕션 빌드는 Railway**에서 자동 실행되며, Linux 컨테이너이므로 이 이슈가 없습니다.
- 굳이 Windows 로컬에서 production build가 필요하면 WSL Ubuntu 환경에서 빌드하세요.
